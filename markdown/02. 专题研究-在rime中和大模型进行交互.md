...
- [ ] 对于rawenglish的运算,实际上当中英中这样的时候,继续添加中文,前面的中英这两部分实际上并不会变化,不用重新计算,只有最后一段中文需要重新计算
- [x] 使用segment.status进行计算更加合理, 我需要监控这个的变化, 然后没有确认的段才需要重新调整
- [x] 英文如果开头,前面应该不需要添加英文部分的前边分隔符.
也就是当内容中是英文开头的时候，关键是如何判断是在内容的最前面.
ok 如果我说可以呢？
AI翻译:ok 这样就可以乐哈哈。
AI翻译:你好我的歌啊
你好 ok 我可以
...
选字的问题先不用处理，可能重构之后，自然就修复了
应该不能随便 清空segment,因为前边已经选好的片段,不能放弃了.

也就是这段完成了之后必须要forward
但问题是这种推进在本轮结束之后,这个seg会消失.
那么只能在下一轮重新forward
