...
- [ ] 云输入法快捷键失效问题, 再服务端没有启动的时候, 云输入法触发按键没有被拦截的问题.
我明白了,这个应该是obsidian的快捷键,obsidian接收到了这个快捷键. 
所以这个也需要注册全局快捷键的方式解决? 
...
- [ ] 如果自动上屏之后,再自动回车,目前好像没找到在rime这边实现的方法,可能还得通过服务端来发送回车.
如果在服务端,应该什么时候发送这个按键呢? 
应该在上屏通知当中,和对端通信吧?
先试试吧,应该有一个可以控制发什么的按钮,

- [ ] 粘贴命令,改成发送完整的state消息,使用sync_with_server

- [ ] 现在如果是自动上屏, 但是是多行? 还是存在问题
...
- [ ] 还有在vscode中会出现错误符号, 应该是shift+Enter中 shift按键导致的.
