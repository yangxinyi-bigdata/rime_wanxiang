...
- [x] (这个似乎不需要因为有可能不用冒号，用户可以自己设置。)考虑把所有ai_assistant配置中的冒号去掉,添加到代码中.
...
- [x] 上屏的空格按键没有拦截啊
- [x] 想一下是不是把云提示词和英文原文上屏的功能,换成两个不同的按键,因为现在有一些不安全感,很容易搞错.
	- [x] 可能获取云输入结果应该用一个不同的快捷键, shift+enter获取云输入法结果, 或者应该可以自己设置
- [x] 现在是直接清空,应该可以通过配置文件控制,不清空,而是正常上屏.
- [ ] 将英文模式换成别的标点符号之后,有兼容性问题
- [ ] 对menu的add_translation, Translation应该是一个由Candidate组成的table,不知道能不能自己合并table,估计够呛.
- [ ] 明天首先把原来electron中有的功能都迁移过来, 恢复正常使用
	- [ ] 然后研究配置动态修改,option动态修改,修改之后马上生效
	- [ ] 现在属性状态的同步总是有延迟,应该要第一时间生效,也就是可以考虑我主动发送一个虚拟快捷键,来让rime马上更新状态.
	- [x] 自动空格上屏
	- [ ] 英文如果开头,前面应该不需要添加英文部分的前边分隔符.
	- [ ] 当有标点符号的时候，上屏的内容中有错误: at:tm da天大地大，世界何处是我家。
	- [ ] 上屏内容前边有空格符,空格好像没有拦截

	- [ ] 最后一段英文模式没有自动确认.

### 核心
自己手动测试的速度太慢了, 我必须开发完整的自动化测试功能.
想办法模拟context这些变量. 看看能不能用二进制存下来.
在python那边应该可以模拟按键来进行自动化测试.

- [ ] at:`ok`woxdxnnijquizvbhde` 进入英文模式失败, 第一段英文,后面再来英文就不行了.
分析: 
...
- [x] nihk`1 " 输入这个的时候, 双引号被替换成了中文双引号,应该在英文模式中跳过的,应该也是bug了
应该是这个坐标范围没有计算对
```
...
这两个 bug 先修复一下。
这两个怎么回事来着？
- [x] 但是现在发现一个新的问题，“中文双引号”的前后两个引号的识别出现了错误。
这个要去进行修复吗? 
搞吧,今天都给它搞定!

- [x] 新重构的英文模式,辅助码会出现问题,明天修复一下
这是辅助码的 bug
```
at:`n
at:`ni
...
- [x] at:`ok`n  这样输入的时候,后面没办法识别到
at:`ok`ni
没有能够自动顶上屏幕, 肯定是因为后面的ni没有被标记成abc导致的.
