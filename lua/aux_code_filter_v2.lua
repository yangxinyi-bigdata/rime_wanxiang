...
-- 创建日志记录器
local logger = logger_module.create("aux_code_filter_v2", {
    enabled = true
})
...
function aux_code_filter.init(env)
    ...
    logger:clear()
    logger:info("aux_code_filter_v2 init")
...
function aux_code_filter.func(translation, env)
    ...
    if not env.single_fuzhu or #input <=2 or #segmente_input % 2 == 0 or context:get_property("backtick_prompt") == 1 then        
    ...
    -- if segmente_input == last_segment_input then
    --     -- 当本轮segmente_input相比上次没有发生任何变化, 说明是选词操作中间的多轮执行,那么就直接跳过后续代码
    --     logger:debug("segmente_input == last_segment_input, 说明是选词中间阶段, 直接返回")
    --     for cand in translation:iter() do
    --         yield(cand)
    --     end
    --     return
    ...
    if has_punctuation then
        logger:debug("有标点符号,直接退出辅助码模式, set_fuzhuma设置为false")
        aux_code_filter.set_fuzhuma = false
        for cand in translation:iter() do
            yield(cand)
        end
    ...
    local success, error_msg = pcall(function()
        ...
        -- last_segment_input = segmente_input
