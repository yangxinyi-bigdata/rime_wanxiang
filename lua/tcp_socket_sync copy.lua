...
-- 连接到Rime状态服务端（快速响应）
function tcp_socket.connect_to_rime_server()
    ...
    if rime_state.client and rime_state.is_connected then
        -- 使用我们的连接检测函数来验证
        if tcp_socket.check_rime_connection() then
            logger.debug("Rime状态服务连接检测通过，无需重连")
            return true
        else
            logger.debug("Rime状态服务连接检测失败，需要重连")
            -- 连接已断开，先断开再重连
            tcp_socket.disconnect_from_rime_server()
        end
    end
...
-- 连接到AI转换服务端（长时间等待）
function tcp_socket.connect_to_ai_server()
    local current_time = get_current_time_ms()
    ...
    if ai_convert.client and ai_convert.is_connected then
        -- 使用我们的连接检测函数来验证
        if tcp_socket.check_ai_connection() then
            logger.debug("AI转换服务连接检测通过，无需重连")
            return true
        else
            logger.debug("AI转换服务连接检测失败，需要重连")
            -- 连接已断开，先断开再重连
            tcp_socket.disconnect_from_ai_server()
        end
    end
...
-- 断开Rime状态服务连接
function tcp_socket.disconnect_from_rime_server()
    local rime_state = socket_system.rime_state
    if rime_state.client then
        pcall(function()
...
-- 断开AI转换服务连接
function tcp_socket.disconnect_from_ai_server()
    local ai_convert = socket_system.ai_convert
    if ai_convert.client then
        pcall(function()
...
-- 断开与所有服务端的连接
function tcp_socket.disconnect_from_server()
    tcp_socket.disconnect_from_rime_server()
    tcp_socket.disconnect_from_ai_server()
...
-- 检测AI转换服务连接状态
function tcp_socket.check_ai_connection()
    local ai_convert = socket_system.ai_convert
...
-- 检测Rime状态服务连接状态
function tcp_socket.check_rime_connection()
    local rime_state = socket_system.rime_state
...
-- 写入Rime状态服务TCP套接字
function tcp_socket.write_to_rime_socket(data)
    ...
    if not rime_state.client or not rime_state.is_connected then
        ...
        if not tcp_socket.connect_to_rime_server() then
    ...
    -- 在发送数据前，先检测连接是否真的可用
    if not tcp_socket.check_rime_connection() then
...
-- 写入AI转换服务TCP套接字
function tcp_socket.write_to_ai_socket(data)
    if not socket_system.is_initialized then
        return false
    end
    ...
    if not ai_convert.client or not ai_convert.is_connected then
        ...
        if not tcp_socket.connect_to_ai_server() then
    ...
    -- 在发送数据前，先检测连接是否真的可用
    if not tcp_socket.check_ai_connection() then
...
function tcp_socket.read_from_rime_socket()
    local rime_state = socket_system.rime_state
    if not rime_state.client or not rime_state.is_connected then
...
function tcp_socket.read_from_ai_socket(timeout_seconds)
    local ai_convert = socket_system.ai_convert
    if not ai_convert.client or not ai_convert.is_connected then
...
function tcp_socket.read_all_from_ai_socket(timeout_seconds)
    local ai_convert = socket_system.ai_convert
    if not ai_convert.client or not ai_convert.is_connected then
        logger.debug("AI转换服务未连接，尝试重新连接...")
        if not tcp_socket.connect_to_ai_server() then
...
function tcp_socket.flush_ai_socket_buffer()
    local ai_convert = socket_system.ai_convert
    if not ai_convert.client or not ai_convert.is_connected then
        logger.debug("AI转换服务未连接，尝试重新连接...")
        if not tcp_socket.connect_to_ai_server() then
...
function tcp_socket.read_latest_from_ai_socket(timeout_seconds)
    local ai_convert = socket_system.ai_convert
    if not ai_convert.client or not ai_convert.is_connected then
        logger.debug("AI转换服务未连接，尝试重新连接...")
        if not tcp_socket.connect_to_ai_server() then
    ...
    if latest_line then
        ...
        -- 尝试解析JSON数据
        local parsed_data = tcp_socket.parse_socket_data(latest_line)
        return {
            data = parsed_data,
            status = "success",
...
-- 解析从Python端接收的数据
function tcp_socket.parse_socket_data(data)
...
-- 处理从Python端接收的命令
function tcp_socket.handle_socket_command(command_messege, env)
    ...
    if command == "ping" then
        ...
        tcp_socket.write_to_rime_socket('{"response": "pong"}')
        return true
    elseif command == "set_option" then
        ...
        if context then
            ...
            tcp_socket.write_to_rime_socket(json.encode(response))
        else
    ...
    elseif command == "get_status" then
        ...
        local stats = tcp_socket.get_stats()
        local response = {
            response = "status",
            data = stats
        }
    ...
    elseif command == "set_config" then
        ...
        local value = command_messege.value

        logger.debug("🔧 收到配置更新命令:")
        logger.debug("   配置名: " .. tostring(config_name))
        logger.debug("   配置路径: " .. tostring(config_path))
        logger.debug("   配置值: " .. tostring(value))
        ...
        local update_success, update_error = pcall(function()
            if type(value) == "boolean" then
                config:set_bool(config_path, value)
            elseif type(value) == "number" then
                if math.floor(value) == value then
                    config:set_int(config_path, value)
                else
                    config:set_double(config_path, value)
                end
            elseif type(value) == "string" then
                config:set_string(config_path, value)
            else
                error("不支持的配置值类型: " .. type(value))
            end
        end)

        if update_success then
            logger.info("✅ 配置更新成功: " .. config_path .. " = " .. tostring(value))
        else
            logger.error("❌ 配置更新失败: " .. tostring(update_error))
        end
...
function tcp_socket.process_rime_socket_data(env)
    local context = env.engine.context
    local data = tcp_socket.read_from_rime_socket()
    if data then
        logger.debug("🎯 成功接收到Rime状态服务完整消息: " .. data)
        local parsed_data = tcp_socket.parse_socket_data(data)
        if parsed_data then
                ...
                if parsed_data.command_messege then
                    if #parsed_data.command_messege > 0 then
                        ...
                        for i, command_item in ipairs(parsed_data.command_messege) do
                            ...
                            if tcp_socket.handle_socket_command(command_item, env) then
                                return true
                            else
                                return false
                            end
                        ...
                        -- 如果是单个命令对象（向后兼容）
                        if tcp_socket.handle_socket_command(parsed_data.command_messege, env) then
                            return true
                        else
                            return false
                        end
...
-- 和Rime状态服务进行数据交换
function tcp_socket.sync_with_server(env, option_info, send_commit_text)
    local context = env.engine.context
    ...
    local success, error_msg = pcall(function()
        ...
        -- 写入Rime状态服务TCP套接字
        tcp_socket.write_to_rime_socket(json_data)

        -- 处理来自Rime状态服务端的数据
        if socket_system.is_initialized and socket_system.rime_state.is_connected then
            tcp_socket.process_rime_socket_data(env)
        end
    end)
...
-- 公开接口：获取连接信息
function tcp_socket.get_connection_info()
    return {
        host = socket_system.host,
        rime_state = {
            port = socket_system.rime_state.port,
...
function tcp_socket.force_reconnect()
    ...
    -- 断开现有连接
    tcp_socket.disconnect_from_server()

    -- 尝试重新连接
    local rime_connected = tcp_socket.connect_to_rime_server()
    local ai_connected = tcp_socket.connect_to_ai_server()
...
-- 公开接口：设置连接参数
function tcp_socket.set_connection_params(host, rime_port, ai_port)
    if host then
...
function tcp_socket.send_convert_request(schema_name, shuru_schema, confirmed_pos_input, long_candidates_table,
    timeout_seconds)
    ...
    local success, error_msg = pcall(function()
        ...
        if json_data then
            -- 写入AI转换服务TCP套接字
            tcp_socket.write_to_ai_socket(json_data)
...
function tcp_socket.read_convert_result(timeout_seconds)
    local timeout = timeout_seconds or 0.1 -- 默认100ms超时，适合流式读取

    -- 使用现有的read_latest_from_ai_socket函数
    local stream_result = tcp_socket.read_latest_from_ai_socket(timeout)
...
function tcp_socket.send_paste_command(env)
    local success, error_msg = pcall(function()
        local current_time = get_current_time_ms()
        ...
        if json_data then
            -- 写入Rime状态服务TCP套接字
            local send_success = tcp_socket.write_to_rime_socket(json_data)
            if send_success then
                logger.info("🍴 粘贴命令发送成功，等待服务端执行")

                -- 可选：等待服务端响应
                local response = tcp_socket.process_rime_socket_data(env)
                if response then
...
function tcp_socket.send_chat_message(commit_text, chat_type, response_key)
    local success, error_msg = pcall(function()
        local current_time = get_current_time_ms()
...
function tcp_socket.send_and_receive_chat(commit_text, context, chat_type, timeout_seconds)
    -- 发送消息
    local send_success = tcp_socket.send_chat_message(commit_text, chat_type)
    if not send_success then
        return nil
    end

    -- 接收流式回复
    return tcp_socket.receive_chat_stream(context, timeout_seconds)
end

-- 初始化系统
function tcp_socket.init()
